...
(목업은 실제 `apps/web/src/styles.css` 팔레트를 재현한 렌더링 — 색 배지가 그 자리에서 발생하는 이벤트)

### 홈 — 추천 레일 + 전체 상품 그리드

<img src="./img/ui-mockup-home.png" alt="홈 화면 목업 — 헤더(이벤트 없음), 추천 레일 3개(imp), 전체 상품 그리드(imp, imp+clk), 뷰포트 경계 아래는 impression 없음" width="620">

### 상품 상세 모달 — 열린 시점에 click은 이미 기록된 상태

<img src="./img/ui-mockup-modal.png" alt="상세 모달 목업 — 닫기·수량 선택은 이벤트 없음, 장바구니 담기=cart, 바로 구매=purchase" width="620">
